 ...
 *                              Number}     auto-detect the available directions. You can also restrict
 *                                          direction by setting this property equal to 'vertical' or
 *                                          'horizontal' or specify an angle in degrees
...
(function(w, m, $, o) {
  ...
  (function() {
    ...
    // damn default implementation rounds -x.5 to -x not x + 1
    var old_round = m.round;
    m.round = function(x) {
      return old_round(m.abs(x)) * m.sign(x);
    }
  }());
  ...
  $.extend(o, {
    ...
    init: function(target, options, data) {
      ...
      if($.inArray(options.direction, ['vertical', 'horizontal', 'multi']) < 0) {
        options.direction = parseFloat(options.direction);
        if(isNaN(options.direction))
          options.direction = 'multi';
        else {
          options.direction = options.direction * Math.PI / 180;
          if (options.direction % 1 == 0)
            options.direction = 'horizontal';
          else if (options.direction % 1 == 0.5)
            options.direction = 'vertical';
          else {
            data.sin = m.sin(options.direction);
            data.cos = m.cos(options.direction);
            data.tan = m.tan(options.direction);
          }
        }
      }
    ...
    toggleDragMode: function(data, dragging) {
      ...
      wheel: function(event, delta) {
        ...
        if(typeof event.data.options.direction == 'number') {
          var scroll = o.computeScroll(event.data, delta * event.data.sin, delta * event.data.cos);
          this.scrollLeft = scroll.left;
          this.scrollTop = scroll.top;
        } else if(event.data.options.wheelDirection === 'horizontal')
    ...
    drag: function(event, ml, mt, left, top) {
      var dx = event.pageX - event.data.position.x;
      var dy = event.pageY - event.data.position.y;
      
      if(typeof event.data.options.direction == 'number') {
        var scroll = o.computeScroll(event.data, dx, dy);
        this.scrollLeft = scroll.left;
        this.scrollTop = scroll.top;
      }
      else {
    ...
    computeScroll: function(data, dx, dy) {
      var dLeftMax = $(data.target).width() + $(data.target).scrollLeft() - $(data.target).attr('scrollWidth');
      var dTopMax = $(data.target).attr('scrollHeight') - $(data.target).height() - $(data.target).scrollTop();
      
      var dLeft = (data.tan * dy + dx) / (m.pow(data.tan, 2) + 1);
      var dTop = data.tan * (data.tan * dy + dx) / (m.pow(data.tan, 2) + 1);
      
      if(m.round(dLeft) < dLeftMax) {
        dLeft = dLeftMax;
        dTop = data.tan * dLeftMax;
      }
      if(m.round(dTop) < -dTopMax) {
        dTop = dTopMax;
        dLeft = dTop / data.tan;
      }
      
      return {
        left: $(data.target).scrollLeft() - m.round(dLeft),
        top: $(data.target).scrollTop() - m.round(dTop)
      };
    }
  });
